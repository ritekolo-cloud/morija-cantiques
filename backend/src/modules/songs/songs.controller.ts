...
export async function getAll(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    const { songs, meta } = await service.getAllSongs(req.query as Record<string, unknown>);
    sendSuccess(res, songs, 'Songs retrieved', 200, meta);
  } catch (err) { next(err); }
}
...
export async function getByNumber(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    const song = await service.getSongByNumber(slug, parseInt(number, 10));
    sendSuccess(res, song);
  } catch (err) { next(err); }
}
...
export async function search(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    const { songs, meta } = await service.searchSongs(req.query as Record<string, unknown>);
    sendSuccess(res, songs, 'Search results', 200, meta);
  } catch (err) { next(err); }
}
